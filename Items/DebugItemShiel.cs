...
namespace Idkmod.Items
{
    class DebugItemShiel : ModItem
    {
        ...
        {
            Main.NewText("ShieldMaxHealth " + player.GetModPlayer<BlPlayer>().shieldMaxHealth + " ShieldCHealth " + player.GetModPlayer<BlPlayer>().shieldCHealth + " GotHit = " + player.GetModPlayer<BlPlayer>().gotHit);
            return true;
        }
    }
}
