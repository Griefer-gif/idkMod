...
namespace idkmod.Projectiles
{
	...
	internal class BBullet : ModProjectile
	{
		...
		public override bool OnTileCollide(Vector2 oldVelocity)
		{
			...
			if (projectile.velocity.X != oldVelocity.X && Math.Abs(oldVelocity.X) > 0.5f)
			{
				projectile.velocity.X = oldVelocity.X * -0.5f;
			}
			if (projectile.velocity.Y != oldVelocity.Y && Math.Abs(oldVelocity.Y) > 0.5f)
			{
