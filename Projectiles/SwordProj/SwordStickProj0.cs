...
{
    public class SwordStickProj0 : ModProjectile
	{

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sword sticky");
		}
		...
		private const int MaxStickSwords = 2; // This is the max. amount of javelins being able to attach
		private readonly Point[] _stickingSwords = new Point[MaxStickSwords]; // The point array holding for sticking javelins
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is SwordStickProj0 javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
					...
					&& javelinProjectile.TargetWhoAmI == target.whoAmI)
				{
					...
					_stickingSwords[currentJavelinIndex++] = new Point(i, currentProjectile.timeLeft); // Add the current projectile's index and timeleft to the point array
					if (currentJavelinIndex >= _stickingSwords.Length)  // If the javelin's index is bigger than or equal to the point array's length, break
						break;
				}
			}

			// Remove the oldest sticky javelin if we exceeded the maximum
			if (currentJavelinIndex >= MaxStickSwords)
			{
				int oldJavelinIndex = 0;
				// Loop our point array
				for (int i = 1; i < MaxStickSwords; i++)
				{
					// Remove the already existing javelin if it's timeLeft value (which is the Y value in our point array) is smaller than the new javelin's timeLeft
					if (_stickingSwords[i].Y < _stickingSwords[oldJavelinIndex].Y)
					{
						oldJavelinIndex = i; // Remember the index of the removed javelin
					}
				}
				// Remember that the X value in our point array was equal to the index of that javelin, so it's used here to kill it.
				Main.projectile[_stickingSwords[oldJavelinIndex].X].Kill();
			}
		}
		...
		private void StickyAI()
		{
			...
			const int aiFactor = 1; // Change this factor to change the 'lifetime' of this sticking javelin
